...
| 4 | **Crown iterated ascent** (structural \(\Phi_n\), regimes \(R_n\), strict \(\vdash\) / \(\not\vdash\) ladder) | [`EPIC_009_STRICT_ITERATED_ASCENT`](specs/INCOMPLETE/IN-PROCESS/EPIC_009_STRICT_ITERATED_ASCENT/EPIC_009_MASTER_ORCHESTRATION.md) | **`SPEC_035_GS1`** → **`SPEC_036_AR1`** → **`SPEC_037_CR1`**; [`007` positioning](docs/007_LIBRARY_POSITIONING_ASSESSMENT.md) §6 | **Not started** |
| 5 | **Expressive / organization frontier** (richer syntax, `initial` derivability, modalities, stronger org) | [`EPIC_010_EXPRESSIVE_ORGANIZATION_FRONTIER`](specs/INCOMPLETE/IN-PROCESS/EPIC_010_EXPRESSIVE_ORGANIZATION_FRONTIER/EPIC_010_MASTER_ORCHESTRATION.md) | **`SPEC_038_XS1`**–**`SPEC_041_OR1`**; [inventory open](docs/THEOREM_INVENTORY.md) | **Not started** |

**Crown milestone vs full active program:** Row **4** is the **crown** track (close **EPIC_009** with **`SPEC_037_CR1`**). Row **5** widens the sentence stack and organization predicates; per epic orchestration it is **not** on the critical path to **`SPEC_037_CR1`** unless a spec records a dependency (e.g. crown proofs that require **`SPEC_038_XS1`** atoms). **Agent contract §4** (“all **active** epics”) still applies: **`EPIC_010`** stays in scope until closed or moved to **Deferred** in [`specs/MASTER_STATUS.md`](specs/MASTER_STATUS.md)—but **ordering** for maximum crown velocity is **finish row 4 in spec order**, then chip at row 5 (or parallelize if staffing allows).
